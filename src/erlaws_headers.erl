...
generate(Host, Method, CanonicalUri, CanonicalQueryString, Payload, AwsRegion, Scope, ExtraHeaders) ->
    ...
    PayloadHash = erlaws_utils:encode_hex(crypto:hash(sha256, Payload)),
    Signature = generate_signature(Method, CanonicalUri, CanonicalQueryString, Headers, PayloadHash, AwsRegion, Scope, Date, Timestamp),
    Authorization = <<"AWS4-HMAC-SHA256 ", "Credential=", Credential/binary, ", ", "SignedHeaders=", SignedHeaders/binary, ", ", "Signature=", Signature/binary>>,
    [{<<"Authorization">>, Authorization}, {<<"x-amz-content-sha256">>, PayloadHash}, {<<"x-amz-date">>, Timestamp} | ExtraHeaders].
...
generate_signature(Method, CanonicalUri, CanonicalQueryString, Headers, PayloadHash, AwsRegion, Scope, Date, Timestamp) ->
    ...
    erlaws_utils:encode_hex(crypto:mac(hmac, sha256, SigningKey, StringToSign)).

generate_canonical_request(Method, CanonicalUri, CanonicalQueryString, Headers, PayloadHash) ->
    Headers1 = lists:sort(Headers),
    <<_:1/binary, CanonicalHeaders/binary>> = lists:foldl(fun({N, V}, Acc) ->
        V2 = if is_integer(V) -> integer_to_binary(V); true -> string:trim(V) end,
        <<Acc/binary, "\n", (string:lowercase(N))/binary, ":", V2/binary>>
    end, <<>>, Headers1),
    <<_:1/binary, SignedHeaders/binary>> = lists:foldl(fun({N, _V}, Acc) ->
        <<Acc/binary, ";", (string:lowercase(N))/binary>> end,
    <<>>, Headers1),
    ...
    erlaws_utils:encode_hex(crypto:hash(sha256, CanonicalRequest)).
